...
# class NSrun():
#     def __init__(self,loglikelihood,prior_bounds, ndims=2, nlive=125,Metropolis=False,num_repeats=125, tol=1e-30,multi_samples=10):
...
#     def multi_sampler(self,loglikelihood,x):
#         avg_val = np.mean([loglikelihood(x) for u in range(self.multi_samples)])
#         return avg_val
...
class NSrun():
    def __init__(self,loglikelihood,prior_bounds, ndims=2, nlive=100,Metropolis=False,num_repeats=100, tol=1e-30,multi_samples=1):
        """orthodox Nested Sampling run"""
        self.ndims = ndims
        self.columns = ['x%i' % i for i in range(ndims)]
        self.tex = {p: '$x_%i$' % i  for i, p in enumerate(self.columns)}
